   ...
   $.when(docsetJsonToXmlDeferred).done(function(docsetXml) { // get data from docset.json and convert to xml format
      if (d2) {
        $.when(d1,d2).done(function (v1,v2) {
           doSaxonCall(mystylesheetLocation, docsetXml, doccentertype, langcode, v1,v2);
        }).fail(function (jqXHR, textStatus, error) {
           doSaxonCall(mystylesheetLocation, docsetXml, doccentertype, langcode);
        });
      } else {
        $.when(d1).done(function (v1) {
           doSaxonCall(mystylesheetLocation, docsetXml, doccentertype, langcode, v1);
        }).fail(function (jqXHR, textStatus, error) {
           doSaxonCall(mystylesheetLocation, docsetXml, doccentertype, langcode);
        });
      }  
...
function doSaxonCall(mystylesheetLocation, mysourcetext, doccentertype, langcode, data1, data2) {
